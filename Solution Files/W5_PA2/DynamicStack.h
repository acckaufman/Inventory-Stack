...
public:
	...
	//###########################################################################	
	//Destructor displays each remaining item in the stack and then deletes it.	#
	//###########################################################################
	~DynamicStack()
	{
		...
		{
			cout << endl << "Now deleting the following item:" << endl;		//Display the data in the item before deleting it
			cout << nodePtr->data;
			cout << endl;
			system("pause");

			nextNode = nodePtr->next;		//Move on to the next item
			delete nodePtr;					//Delete the item just displayed
			nodePtr = nextNode;
		}
	}
	...
	void displayStack()
	{
		...
		cout << endl << "Here are the values currently in the stack:" << endl;
